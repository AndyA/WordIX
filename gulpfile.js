...
function flatten() {
  ...
  for (var i = 0; i < arguments.length; i++) {
    var elt = arguments[i];
    if (Array.isArray(elt)) Array.prototype.push.apply(out, elt);
    else out.push(elt);
  }
  return out;
}
...
gulp.task("browser-sync", function() {
  ...
  gulp
    .watch(["lib/**/*.pm", "components/*/lib/**.pm", "config.yml"])
    .on("change", function() {
      setTimeout(browserSync.reload, 3000);
    });

  gulp.watch(["views/**/*.tt"]).on("change", browserSync.reload);
});
...
gulp.task("watchless", ["less"], function() {
  var watchFiles = flatten(
    configFile,
    paths.less,
    paths.less_libs.map(function(dir) {
        ...
        var bundle = function() {
          ...
          return bundler
            ...
            .on("end", function() {
              ...
              log(
                "Finished " + mode + " bundler for",
                colors.cyan(file.relative),
                "after",
                colors.magenta(prettyHrtime(endTime))
              );
            })
...
gulp.task("tdd", ["test"], function() {
  var watchfiles = flatten(
    "build/js/**/*.js",
    paths.test,
    paths.js_libs.map(function(dir) {
...
gulp.task("server:development", function() {
  ...
  gulp.watch(["bin/app.js", "views/**/*.hbs", "webapp/**/*.js"], function() {
    browserSync.reload();
    server.start();
