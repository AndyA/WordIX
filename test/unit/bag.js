...
describe("Bag", () => {
  ...
  describe("pull", () => {
    ...
    it("should remove tiles from the bag", () => {
      expect(bag.size).to.equal(100 - pick.length);
    });
  });
  ...
  describe("take", () => {
    ...
    // Mark the bag
    for (let i in bag.tiles) bag.tiles[i]._index = i;
    ...
    it("should be a random selection", () => {
      ...
      expect(s).to.be.above(15);
      expect(m).to.be.above(bagSize / 4);
      expect(m).to.be.below((bagSize * 3) / 4);
    });
    it("should remove tiles from the bag", () => {
      expect(bag.size).to.equal(100 - got.length);
    });
  });
});
