...
const { Tile, WildTile, makeTile } = require("../../webapp/lib/js/tile.js");
...
describe("Rules", () => {
  describe("board", () => {
    ...
    function getSpecial(sp) {
      ...
      if (sp.length === 1) {
        const spr = sp[0];
        if (spr.multiplier < 2 || spr.multiplier > 3) return "M";
        const letter = spr.multiplier === 2 ? "d" : "t";
        if (spr.scope === "word") return letter.toUpperCase();
        if (spr.scope === "letter") return letter.toLowerCase();
        return "S";
      }
      return "*";
    }
    ...
    it("should have a familar board", () => {
      expect(scanBoard(r.makeBoard())).to.deep.equal(expectBoard);
    });
  });
  ...
  describe("bag", () => {
    ...
    it("should have two wild tiles", () => {
      ...
      expect(wild.length).to.equal(2);
      expect(wild[0]).to.be.an.instanceof(WildTile);
    });

    it("should have three Gs with a score of two", () => {
      const gs = bag.tiles.filter(t => t.matchLetter === "G");
      expect(gs.length).to.equal(3);
      expect(gs[0].score).to.equal(2);
    });
  });
  ...
  describe("eachValid", () => {
    ...
    it("should return the correct opening moves", () => {
      expect(catchValid()).to.deep.equal(firstMove);
    });

    it("should return the correct second move", () => {
      b.cell(0, 3).tile = makeTile("T");
      b.cell(1, 3).tile = makeTile("H");
      b.cell(2, 3).tile = makeTile("E");

      const nextMove = catchValid(2);

      expect(sortMoves(nextMove)).to.deep.equal(sortMoves(secondMove));
    });
  });
});
