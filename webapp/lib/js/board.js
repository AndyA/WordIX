...
class Cell extends MW.mix(Object).with(Generation) {
...
class Board extends MW.mix(Object).with(Generation) {
  constructor(opt) {
    super();
    Object.assign(this, { width: 15, height: 15 }, opt);

    if (!this.board) this.clear();
  }
  ...
  _emptyBoard() {
    ...
    for (let y = 0; y < this.height; y++) {
      let row = [];
      for (let x = 0; x < this.width; x++) row.push(new Cell(this));
      board.push(row);
    }
    return board;
  }
  ...
  get used() {
    ...
    for (const row of this.board || [])
      for (const cell of row) if (cell.tile) count++;
    return count;
  }
  ...
  eachRow(func) {
    for (let y = 0; y < this.height; y++) func(this.board[y], y);
  }

  each(func) {
    this.eachRow((row, y) => {
      for (let x = 0; x < this.width; x++) func(row[x], x, y);
    });
  }
