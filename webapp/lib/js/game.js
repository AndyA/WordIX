...
class Game extends MW.mix(Object).with(Generation) {
  constructor(opt) {
    super();
    Object.assign(this, { players: 2, next: 0, skipped: 0 }, opt);
    if (!this.rules) throw new Error("Missing option: rules");
    if (!this.trie) throw new Error("Missing option: trie");
  ...
  _init() {
    ...
    for (const pl of this.players) this.fillTray(pl);
  }

  get canPlay() {
    if (this.skipped >= this.players.length) return false;
    for (const p of this.players) if (p.tray.size === 0) return false;
    return true;
  }

  startMove() {
    this.currentPlayer = this.players[this.next++];
    if (this.next == this.players.length) this.next = 0;
  ...
  sanityCheck() {
    ...
    if (invalid.length) throw new Error("Illegal words: " + invalid.join(", "));
  }
